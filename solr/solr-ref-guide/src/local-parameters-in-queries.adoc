...
If no "type" is specified (either explicitly or implicitly) then the <<standard-query-parser.adoc#standard-query-parser,lucene parser>> is used by default. Thus
