...
`maxChars`::
The upper limit for the number of characters to be copied. The section <<copy-fields.adoc#copy-fields,Copy Fields>> has more details.
...
The output will include the `source` and `dest` (destination) of each copy field rule defined in `schema.xml`. For more information about copy fields, see the section <<copy-fields.adoc#copy-fields,Copy Fields>>.
