...
|===
| <<datadir-and-directoryfactory-in-solrconfig.adoc#datadir-and-directoryfactory-in-solrconfig,DataDir and DirectoryFactory in SolrConfig>>: Where and how Solr's indexes are stored.
| <<schema-factory-definition-in-solrconfig.adoc#schema-factory-definition-in-solrconfig,Schema Factory Definition in SolrConfig>>: Schema file formats.
| <<indexconfig-in-solrconfig.adoc#indexconfig-in-solrconfig,IndexConfig in SolrConfig>>: Options for Lucene index writers, including segment management, merges, and locks.
...
| <<requesthandlers-and-searchcomponents-in-solrconfig.adoc#requesthandlers-and-searchcomponents-in-solrconfig,RequestHandlers and SearchComponents in SolrConfig>>: *TODO*
| <<implicit-requesthandlers.adoc#implicit-requesthandlers,Implicit RequestHandlers>>: Request end-points automatically provided by Solr and how to configure them.
| <<initparams-in-solrconfig.adoc#initparams-in-solrconfig,InitParams in SolrConfig>>: Default parameters for request handlers.
| <<updatehandlers-in-solrconfig.adoc#updatehandlers-in-solrconfig,UpdateHandlers in SolrConfig>>: Update requests and commit settings.
| <<query-settings-in-solrconfig.adoc#query-settings-in-solrconfig,Query Settings in SolrConfig>>: Caches, query warming, and query listeners.
| <<requestdispatcher-in-solrconfig.adoc#requestdispatcher-in-solrconfig,RequestDispatcher in SolrConfig>>: Advanced settings for how parsing requests and HTTP cache headers.
| <<update-request-processors.adoc#update-request-processors,Update Request Processors>>: Plugins for update requests. *TODO*
| <<script-update-processor.adoc#script-update-processor,Script Update Processor>>: Java scripting engines during document updates.
| <<codec-factory.adoc#codec-factory,Codec Factory>>: The Lucene codec to use when writing data to disk.
