...
(For more information about the `copyField` directive, see <<fields-and-schema-design.adoc#fields-and-schema-design,Fields and Schema Design>>.)
...
In addition to some of the <<Local Parameters for Faceting,general local parameters>> supported by other types of faceting, a `stats` local parameters can be used with `facet.pivot` to refer to <<stats-component.adoc#stats-component,`stats.field`>> instances (by tag) that you would like to have computed for each Pivot Constraint.
