...
Note that this screen only changes logging for the Solr instance that is serving the UI.
If there are multiple instances in a cluster, the log levels will not be changed on all nodes.
Log levels will also be reset on next Solr restart.
...
Click the current level in the right column and the Log Level Menu will appear.
Select the desired level and the selection will be saved immediately.
