...
The Extended DisMax (eDisMax) query parser is an improved version of the <<dismax-query-parser.adoc#dismax-query-parser,DisMax query parser>>.

In addition to supporting all the DisMax query parser parameters, Extended DisMax:

* supports <<standard-query-parser.adoc#standard-query-parser,Solr's standard query parser>> syntax such as (non-exhaustive list):
